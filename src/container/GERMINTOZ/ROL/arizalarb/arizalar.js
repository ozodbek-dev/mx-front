...
const Rolarizab = () => {
  ...
  const handleChange = (event) => {
   ...
   function Heets(e) {
    ...
    fordata.append("kimga", check);
    const numarrsend2 = []
    numarr.map((item)=> {
      numarrsend2.push(`${Number(item.from)}-${Number(item.to)}`)
    })
    fordata.append('yosh_toifa', numarrsend2)
    const numarrsend1 = []
    numarr2.map((item)=> {
      numarrsend1.push(`${Number(item.from)}-${Number(item.to)}`)
    })
    fordata.append('oy_toifa', numarrsend1)
          ...
          <div className="rol_ariza_bottom_bigbox">
            ...
            <div className="rol_ariza_bottom_block">
              ...
              <div className="muassasa_block">
                <h6 className="muassasa_title">{t("vosita.muas")}</h6>
                ...
                <div
                  ...
                  style={
                    opens === true ? { display: "block" } : { display: "none" }
                  }
                >
                  <TextField
                    id="outlined-basic"
